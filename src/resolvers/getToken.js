  ...
  return new Promise((resolve, reject) => {
    mysqlClient.query(`SELECT * FROM Users WHERE username LIKE '${username}' OR email LIKE '${username}'`, (err, result) => {
      ...
      bcrypt.compare(password, user.password, async (err, compareSuccess) => { // compare the provided password with the user one
        ...
        // save refresh token
        try {
          await cache(`session:${user.id}:${refresh_token.value}`, null, JSON.stringify({createdAt: refresh_token.createdAt, expiresAt: refresh_token.expiresAt, userLocation: geo, userPlatform: platformData}), process.env.REFRESH_TOKEN_DURATION * 24 * 60 * 60, true, true, redisClient);
        } catch(e) {
