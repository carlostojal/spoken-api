...
const resolvers = {
  Query: {
    ...
    getToken: async (parent, args, context, info) => {
      const tokens = await getToken(args.username, args.password, context.req.headers["user-agent"], context.redisClient);
    ...
    refreshToken: async (parent, args, context, info) => {
      ...
      const tokens = await refreshToken(refresh_token, context.req.headers["user-agent"], context.redisClient);
